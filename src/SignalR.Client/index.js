const url = window.API_BASE_URL || "http://localhost:8080";
...
async function initializeSubscriberConnection(subscriptionId) {
  var connection = new signalR.HubConnectionBuilder()
    .withUrl(`${url}/hub?subscriptionId=${subscriptionId}`)
    .build();
...
const subscribePageSubscriptionContainer = document.querySelector(
  "#subscribe-page-subscription"
);
const subscribePageListContainer = document.querySelector(
  "#subscribe-page-list"
);

// Load buttons
const mainPageBuyButton = document.querySelector("#main-page-buy-button");
const mainPageSubscribeButton = document.querySelector(
  "#main-page-subscribe-button"
);
const buyPageBackButton = document.querySelector("#buy-page-back-button");
const subscribePageBackButton = document.querySelector(
  "#subscribe-page-back-button"
);
const subscribePageSubscribeButton = document.querySelector(
...
const subscribePageProductsList = document.querySelector(
  "#subscribe-page-products-list"
);
const subscribePageSubscribeInput = document.querySelector(
  "#subscribe-page-subscribe-input"
);
//#endregion
const buyPageSubscriptionId = document.querySelector(
  "#buy-page-subscription-id"
);
...
mainPageBuyButton.addEventListener("click", async () => {
  ...
  const productsListElement = document.querySelectorAll(
    ".buy-page-products-list-element"
  );
  ...
  productsListElement.forEach((element) => {
    element.addEventListener("click", async () => {
      ...
      else boughtProducts.set(productId, 1);
      ...
      connection.invoke(
        "PublishBoughtProducts",
        subscriptionId,
        Object.fromEntries(boughtProducts)
      );

      const buyListElement = document.querySelectorAll(
        ".buy-page-buy-list-element"
      );
